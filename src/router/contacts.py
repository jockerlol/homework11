...
async def create_contact(contact: ContactRequest, db: Session = Depends(get_db)):
    if db.query(Contact).filter_by(phone_number=contact.phone_number).first():
        raise HTTPException(
            status_code=400, detail="The phone number is already in the database"
        )
...
async def update_contact(
    contact_id: int, updated_contact: ContactRequest, db: Session = Depends(get_db)
):
    contact = db.query(Contact).filter(Contact.id == contact_id).first()
    if contact is None:
        raise HTTPException(status_code=404, detail="Contact is missing")
...
async def delete_contact(contact_id: int, db: Session = Depends(get_db)):
    ...
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact is missing"
        )
    else:
        db.delete(contact)
        db.commit()
...
async def future_birthdays(db: Session = Depends(get_db)):
    ...
    future_birthdays_in_this_year = (
        db.query(Contact)
        .filter(text("TO_CHAR(birthday, 'MM-DD') BETWEEN :start_date AND :end_date"))
        .params(
            start_date=today.strftime("%m-%d"),
            end_date=today_plus_seven_days.strftime("%m-%d"),
        )
        .all()
    )

    return future_birthdays_in_this_year